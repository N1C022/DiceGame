...
class Application(tk.Frame):
  def __init__(self, master=None):
      ...
      self.level = tk.StringVar(value='GODLY')
      self.level_dict = {'GODLY':'GODLY'}
      self.mine_dict = {'GODLY':15}
  ...
  def draw_main_frame(self):
      ...
      self.logo = tk.Label(self.main_frame, image=ATHENASCHALLENGE_logo)
  ...
  def updateAdjecentCells(self, btn, row, col):
      ...
      if num:
          ...
          btn['fg'] = color
          btn['text'] = str(num)
  def game_won_window(self):
      self.top = tk.Toplevel(self)
      self.top.geometry('300x100+517+355')
      self.top.title('ATHENASCHALLENGE')
      self.top.resizable(0,0)
      self.top.protocol("WM_DELETE_WINDOW", self.master.destroy)

      tk.Label(self.top, text=' You Won!!', fg='black',
              font=('verdana', 15, 'bold'), compound=tk.LEFT,
              ).grid(row=0, column=0, padx=50, pady=5,
                      columnspan=4)
      tk.Label(self.top, text='ATHENA IS DELIGHTED', fg='black',
  ...
  def game_lost_window(self):
      ...
      tk.Label(self.top, text=' You Lost', fg='black',
              font=('verdana', 15, 'bold'), compound=tk.LEFT,
              ).grid(row=0, column=0, padx=50, pady=5,
                      columnspan=4)
      tk.Label(self.top, text='ATHENA IS DISSAPOINTED', fg='black',
              font=('verdana', 10, 'bold'), compound=tk.LEFT,
              ).grid(row=5, column=0, padx=50, pady=5,
                      columnspan=4)
      complete=False
  ...
  def help_window(self):
      ...
      win.geometry('200x180+580+355')
      win.title('ATHENASCHALLENGE')
      win.resizable(0,0)
...
def main():
  ...
  root.title('ATHENASCHALLENGE')
  root.geometry('345x450+500+150')
  global mine_icon, ATHENASCHALLENGE_logo
